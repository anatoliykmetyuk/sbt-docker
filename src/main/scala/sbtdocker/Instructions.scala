...
/** Helper for product classes. Sets instruction name from the class name and uses product values as arguments.
  */
...
/** Instruction that both stages files to the staging directory and adds a Dockerfile instruction.
  *
  * A [[sbtdocker.staging.DockerfileProcessor]] is used to stage files in the staging directory, which also requests the Dockerfile
  * instruction when the file paths have been finalized.
  */
trait FileStagingDockerfileInstruction extends FileStagingInstruction {
  ...
  /** Creates a Dockerfile instruction given the final paths of the sources.
    *
    * @param sources
    *   Paths to the sources in the staging directory.
    * @return
...
object Instructions {
  ...
  /** Sets label(s) for the image. Docker Version >= 1.6 required. Example: "A=1 B=2 C=3"
    * @param labels
    *   image labels
    */
  ...
  object Cmd {
    ...
    /** Command to execute through a shell (`/bin/sh`) when the image starts, or default arguments to the entry point.
      * @param commands
      *   Command list.
      */
  ...
  /** When an entry point is specified `CMD` acts as default arguments that is appended the the entry point command. If an entry point is
    * not specified `CMD` will be the default command for the image.
    * @param command
    *   Command.
    */
  ...
  /** Sets environment variables in the image. Example: "A=1 B=2 C=3"
    * @param variables
  ...
  /** Adds files to the image. If a source file is a tar file it will be extracted as a directory.
    * @param sources
    *   Source files.
    * @param destination
    *   Destination path inside the container.
    * @param chown
    *   Set the owner user and group of the files in the container. Added in 17.09.0-ce.
    */
  ...
  /** Adds files from the staging directory to the image. If a source file is a tar file it will be extracted as a directory.
    * @param sources
    *   Source path inside the staging directory.
    * @param destination
    *   Destination path inside the container.
    * @param chown
    *   Set the owner user and group of the files in the container. Added in 17.09.0-ce.
  ...
  /** Adds files from the staging directory to the image.
    * @param sources
    *   Source path inside the staging directory. Cannot be empty.
    * @param destination
    *   Destination path inside the container.
    * @param chown
    *   Set the owner user and group of the files in the container. Added in 17.09.0-ce.
  ...
  /** Add mount points inside the container.
    * @param paths
    *   Paths inside the container. Cannot be empty.
    */
  ...
  /** Stages files. Only adds files and directories to the staging directory, will not yield an instruction in the Dockerfile. To use the
    * staged files use the [[sbtdocker.Instructions.AddRaw]] and [[sbtdocker.Instructions.CopyRaw]] instructions.
    * @param sources
    *   Source files.
    * @param destination
    *   Destination path.
  ...
  /** Defines health check command to run inside the container to check that it is still working.
    ...
    *   check completes.
    * @param timeout
    *   If a single run of the check takes longer than specified then the check is considered to have failed.
    * @param startPeriod
    *   Provides initialization time for containers that need time to bootstrap.
    * @param retries
    *   How many consecutive failures of the health check for the container to be considered unhealthy.
    */
  ...
  /** This class allows the user to specify a raw Dockerfile instruction. Example:
    * {{{
    * Raw("RUN", "echo '123'")
